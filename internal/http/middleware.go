...
func (w *hijackResponseWriter) Write(data []byte) (n int, err error) {
	...
	case http.StatusNotFound:
		data, err = json.Marshal(ErrorResponse{Err: http.StatusText(http.StatusNotFound)})
	case http.StatusMethodNotAllowed:
		data, err = json.Marshal(ErrorResponse{Err: http.StatusText(http.StatusMethodNotAllowed)})
	}
	if err != nil {
		return 0, err
	}
